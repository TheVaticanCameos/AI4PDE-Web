...
model_path: str = os.path.join(root_dir, 'back-end', 'params',
                               'dr-params.ckpt-20000.pt')
...
sys.path.append(
    os.path.join(root_dir, 'back-end', 'source', 'diffusion-reaction'))
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # 将文件内容转换为numpy数组
        data = np.loadtxt(filepath, delimiter=',') if filename.endswith(
            '.csv') else np.loadtxt(filepath)
